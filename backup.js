...
class DuckChainAPIClient {
  ...
  async setDuckName(authorization, duckName) {
    try {
      ...
      const response = await axios.get(
        `https://preapi.duckchain.io/user/set_duck_name?duckName=${encodedDuckName}`,
        {
          headers: {
            ...this.headers,
  ...
  async completeTask(authorization, task) {
    try {
      ...
      if (response.data.code === 200) {
        this.log(
          `Làm nhiệm vụ ${task.content} thành công | Phần thưởng: ${task.integral} DUCK`,
          "success"
        );
        return { success: true, data: response.data.data };
      } else {
  ...
  async processAllTasks(authorization) {
    try {
      ...
      if (!taskInfo.success) {
        this.log(
          `Không thể lấy thông tin nhiệm vụ: ${taskInfo.error}`,
          "error"
        );
        return;
      }
      ...
      if (!taskList.success) {
        this.log(
          `Không thể lấy danh sách nhiệm vụ: ${taskList.error}`,
          "error"
        );
        return;
      }
      ...
      if (partner && Array.isArray(partner)) {
        for (const task of partner) {
          if (!completedPartner.includes(task.taskId)) {
            this.log(
              `Đang thực hiện nhiệm vụ đối tác: ${task.content}...`,
              "info"
            );
            await this.completeTask(authorization, task);
            await new Promise((resolve) => setTimeout(resolve, 2000));
  ...
  async executeQuack(authorization) {
    try {
      ...
      if (response.data.code === 200) {
        ...
        this.log(
          `Quack lần ${quackTimes} | Tổng âm: ${totalNegative} | Decibel còn lại: ${decibel}`,
          "custom"
        );
        return { success: true, data: response.data.data };
      } else {
  ...
  async processQuacks(authorization, decibels, maxQuackTimes = 0) {
    ...
    while (
      decibels > 0 &&
      (maxQuackTimes === 0 || quackCount < maxQuackTimes)
    ) {
  ...
  async main() {
    ...
    const quacktime = await this.askQuestion(
      "Bạn có muốn Quack Times không? (y/n)..Quack Times có thể sẽ bị trừ hết DUCK: "
    );
    const hoiquacktime = quacktime.toLowerCase() === "y";
    let maxQuackTimes = 0;

    if (hoiquacktime) {
      const quackTimesInput = await this.askQuestion(
        "Bạn muốn Quack Times bao nhiêu lần? (Nhấn Enter để quack đến hết): "
      );
        ...
        if (userInfo.success) {
          ...
          if (userInfo.data.duckName === null) {
            ...
            if (setNameResult.success) {
              this.log(
                `Đặt tên duck thành công: ${setNameResult.data.duckName}`,
                "success"
              );
              ...
              if (hoiquacktime && setNameResult.data.decibels > 0) {
                await this.processQuacks(
                  authorization,
                  setNameResult.data.decibels,
                  maxQuackTimes
                );
              }
            } else {
              this.log(
                `Không thể đặt tên duck: ${setNameResult.error}`,
                "error"
              );
            }
          } else {
            this.log(
              `Duck name đã được thiết lập: ${userInfo.data.duckName}`,
              "info"
            );
          ...
          this.log(
            `Không thể lấy thông tin tài khoản: ${userInfo.error}`,
            "error"
          );
        }
